...
import { Building2, MapPin, Layers, Briefcase, UserCheck } from "lucide-react";
...
export default async function ContractorDashboard() {
  ...
  // Query projects linked to this contractor via RLS
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Welcome, <span className="text-white font-medium">{profile?.full_name}</span> — Linked Projects &amp; Work Execution
            </p>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              <span>Linked Development Projects ({projects?.length || 0})</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects && projects.length > 0 ? (
              projects.map((project) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3 bg-amber-500/5 rounded-xl border border-amber-500/10 text-xs text-amber-300">
                      Mobile work submissions and photo progress tracking will activate in Phase 6.
                    </div>
                  </div>
                );
              })
              ...
              <div className="col-span-full py-16 text-center bg-white/5 rounded-2xl border border-dashed border-white/10 p-6">
                ...
                <h3 className="text-lg font-semibold text-slate-300">No Projects Linked</h3>
                <p className="text-slate-500 text-xs max-w-sm mx-auto mt-1">
                  You are not currently linked as a contractor on any active projects.
                </p>
              </div>
            )}
          </div>
